...
CACHE_DIR = "./donor_cache_ag_all_gene"
CACHE_VERSION = "v1.1"
...
def prepare_data_cached(use_gene_list=True):
    """缓存优化的数据准备函数"""
    ...
    try:
        ...
        donor_info = pd.read_csv("donor_metadata_no_siage_children.csv", index_col=0)
        if use_gene_list:
            df_gene_list = pd.read_csv("valid_gene_list.csv", index_col=0, header=None)
        donor_files_dir = "donor_files_all_gene"
            ...
            if donors:
                ...
                if use_gene_list:
                    pseudobulk_data[dataset_name] = get_pseudobulk_from_donor_files_cached(
                        donor_files_dir, donors, dataset_name, gene_names=df_gene_list.index, use_raw=USE_RAW
                    )
                else:
                    pseudobulk_data[dataset_name] = get_pseudobulk_from_donor_files_cached(
        ...
        if use_gene_list:
            train_cell_data, train_max_cells = load_single_cell_data_cached(
                donor_files_dir, train_donors_subset, "training_subset", gene_names=df_gene_list.index, use_raw=USE_RAW
            )
            test_cell_data, test_max_cells = load_single_cell_data_cached(
                donor_files_dir, test_donors_subset, "testing_subset", gene_names=df_gene_list.index, use_raw=USE_RAW
            )
        else:
            train_cell_data, train_max_cells = load_single_cell_data_cached(
...
def train_autogluon_with_deepsets(data):
    ...
    predictor = TabularPredictor(
        label="age",
        problem_type="regression", 
        path="AutogluonModels/deepsetsattn_integration_all_gene",
        verbosity=2,
    ...
    hyperparameters = {
        ...
        DeepSetsTabularModelAttn: {'hidden_dim': 11, 'dropout': 0.2, 'lr': 0.001, 'batch_size': 8, 'epochs': EPOCH, 'cache_every_epoch': 5} # Tid
    }
    ...
    try:
        ...
        df_all = save_all_predictions(standard_preds, deepsets_preds, save_dir="./results_all_gene_wo_siAge_children_ig_cells")
        
        # 🔥 新增：创建可视化对比
        visualize_prediction_comparison(df_all, save_dir="./results_all_gene_wo_siAge_children_ig_cells")
...
#                 save_dir="./results_all_gene_wo_siAge_children_ig_cells",
...
def save_all_predictions(standard_preds, deepsets_preds, save_dir="./results_all_gene_wo_siAge_children_ig_cells"):
    """
...
def visualize_prediction_comparison(df_all, save_dir="./results_all_gene_wo_siAge_children_ig_cells"):
    """可视化预测结果对比"""
...
def cache_management():
    ...
    if choice == '1':
        ...
        # 显示DeepSets权重缓存信息
        from pathlib import Path
        weights_dir = Path("./results_all_gene_wo_siAge_children_ig_cells/deepsets_weights")
...
def analyze_deepsets_contributions(predictor, data, donor_ids_to_analyze=None):
    ...
    try:
        ...
        cell_contrib = deepsets_model.get_cell_contributions(
            donor_ids_to_analyze, method='gradient' # Tid: gradient, grad_input, attention, activation, integrated_gradient
        )
        ...
        gene_contrib = deepsets_model.get_gene_contributions(
            donor_ids_to_analyze, gene_names=gene_names, method='gradient' # Tid: gradient, grad_input, integrated_gradient
        )
        ...
        for i, donor_id in enumerate(donor_ids_to_analyze[:2]):
            ...
            deepsets_model.visualize_contributions(
                cell_contrib, donor_id, plot_type='cell', save_dir="./results_all_gene_wo_siAge_children_ig_cells", signed=True
            )
            deepsets_model.visualize_contributions(
                gene_contrib, donor_id, plot_type='gene', gene_names=gene_names, save_dir="./results_all_gene_wo_siAge_children_ig_cells", signed=True
            )
        ...
        print(f"📁 Visualizations saved in ./results_all_gene_wo_siAge_children_ig_cells/", flush=True)
...
def main():
    ...
    os.makedirs("./results_all_gene_wo_siAge_children_ig_cells", exist_ok=True)
    ...
    try:
        ...
        # 数据准备（带缓存）
        data = prepare_data_cached() if 'all_gene' in CACHE_DIR else prepare_data_cached(use_gene_list=False)
        ...
        print("📁 Results saved to ./results_all_gene_wo_siAge_children_ig_cells/", flush=True)
...
if __name__ == "__main__":
    ...
    if results[0]:  # 如果有predictor
        ...
        print("  Check ./results_all_gene_wo_siAge_children_ig_cells/ for plots and contribution analysis", flush=True)
        ...
        if contrib_results:
            ...
            print("  - Visualization saved in ./results_all_gene_wo_siAge_children_ig_cells/", flush=True)
